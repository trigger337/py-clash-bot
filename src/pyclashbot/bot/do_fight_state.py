...
def do_2v2_fight_state(
    ...
    if wait_for_2v2_battle_start(controller, logger=logger) is not True:
    ...
    if (
        ...
        and _2v2_fight_loop(controller, logger=logger) == "restart"
    ):
    ...
    if (
        random_fight_mode
        and _2v2_random_fight_loop(controller, logger=logger) == "restart"
    ):
...
def do_1v1_fight_state(
    controller: BaseEmulatorController, logger: Logger, next_state, random_fight_mode
):
    ...
    # wait for battle start
    if wait_for_1v1_battle_start(controller, logger) == "restart":
        logger.change_status(
    ...
    if not random_fight_mode and _1v1_fight_loop(controller, logger) == "restart":
        logger.log("Error 884458245 Failuring in fight loop")
        return "restart"

    # run random fight loop if random mode toggled
    if random_fight_mode and _1v1_random_fight_loop(controller, logger) == "restart":
...
def check_if_on_path_of_legends_mode(controller: BaseEmulatorController):
...
def start_2v2_fight_state(
    controller: BaseEmulatorController, logger: Logger
) -> Literal["restart", "2v2_fight"]:
    ...
    if get_to_challenges_tab_from_main(controller, logger) == "restart":
        return "restart"

    # check for then close popup
    if check_for_challenge_page_on_events_tab(controller):
        close_this_challenge_page(controller)
        time.sleep(1)

    # scroll up
    for _ in range(10):
        controller.swipe((66, 300), (66, 400))
    time.sleep(1)

    # if there is a locked events page, return restart
    if check_for_locked_events_page(controller):
        logger.change_status("Locked events page! Doing 1v1 instead...")
        controller.click((170, 589))
        time.sleep(3)
        if check_if_on_clash_main_menu(controller) is not True:
            ...
            return "restart"
        return start_1v1_fight_state(controller, logger)

    # click 2v2 icon location
    click_2v2_icon_button(controller)
    time.sleep(1)

    # click battle button
    click_2v2_battle_button(controller)
    time.sleep(1)

    # click quickmatch button
    click_quickmatch_button(controller)
    time.sleep(1)

    return next_state


def check_for_locked_events_page(controller: BaseEmulatorController):
    iar = controller.screenshot()
    pixels = [
        iar[254][93],
        iar[122][240],
...
def start_path_of_legends_1v1_state(
    controller: BaseEmulatorController, logger: Logger, next_state
):
    ...
    if not check_if_on_path_of_legends_mode(controller):
        logger.change_status("Switching to path of legends mode")
        controller.click((280, 400))
        time.sleep(2)
    ...
    controller.click((207, 400))

    return next_state


def start_1v1_fight_state(
    controller: BaseEmulatorController, logger: Logger, mode="trophy_road"
    ...
    if mode == "path_of_legends":
        ...
        return start_path_of_legends_1v1_state(controller, logger, next_state)
    else:
        logger.change_status("Starting a tropy road battle")
        logger.increment_trophy_road_fights()
        if start_trophy_road_fight(controller, logger) is False:
            return "restart"

    return next_state


def start_trophy_road_fight(controller: BaseEmulatorController, logger) -> bool:
    ...
    if check_if_on_path_of_legends_mode(controller):
...
def check_for_challenge_page_on_events_tab(controller: BaseEmulatorController):
...
def close_this_challenge_page(controller: BaseEmulatorController) -> None:
    """method to close an ongoing challenge page on the events tab"""

    controller.click(CLOSE_THIS_CHALLENGE_PAGE_BUTTON)


def click_2v2_icon_button(controller: BaseEmulatorController) -> None:
    """method to click the 2v2 icon on the challenges tab"""

    controller.click(_2V2_BATTLE_ICON_COORD)


def click_2v2_battle_button(controller: BaseEmulatorController) -> None:
    """method to click the 2v2 battle button on the challenges tab"""
    controller.click(_2V2_BATTLE_BUTTON_COORD_2)


def click_quickmatch_button(controller: BaseEmulatorController) -> None:
    """method to click the quickmatch button on the 2v2 battle screen"""

    controller.click(QUICKMATCH_BUTTON_COORD)


def choose_play_side_barebones(
    controller: BaseEmulatorController, favorite_side
):  # -> Any | Literal['right', 'left']:
    """method to choose a play side given a favorite side"""

    # get tower_statuses
    tower_statuses = check_enemy_tower_statuses(controller)
...
def emote_in_2v2(controller: BaseEmulatorController, logger: Logger) -> Literal["good"]:
    """method to do an emote in a 2v2 match"""

    logger.change_status("Hitting an emote")

    # click emote button
    controller.click(EMOTE_BUTTON_COORD_IN_2V2)
    time.sleep(0.33)

    emote_coord = random.choice(EMOTES_COORDS_IN_2V2)
    controller.click(emote_coord)
    time.sleep(0.33)

    return "good"


def check_if_at_max_elixer(controller: BaseEmulatorController):
    iar = controller.screenshot()
    pixels = [
        iar[612][304],
        iar[612][312],
...
def mag_dump(controller: BaseEmulatorController, logger):
    card_coords = [
        (137, 559),
        (206, 559),
        (274, 599),
    ...
    for index in range(3):
        ...
        controller.click(card_coord)
        time.sleep(0.1)

        controller.click(play_coord)
        time.sleep(0.1)


def wait_for_4_elixer(controller: BaseEmulatorController, logger, mode="1v1"):
    ...
    while 1:
        ...
        if len(check_which_cards_are_available(controller)) == 4:
            logger.change_status("All cards are available!")
            return True

        if check_for_4_elixer(controller):
            break

        if check_if_at_max_elixer(controller):
            return "mag_dump"

        if time.time() - start_time > ELIXER_WAIT_TIMEOUT:
            return "restart"

        if mode == "1v1" and not check_for_in_battle_with_delay(controller):
            logger.change_status(status="Not in battle, stopping waiting for 4 elixer.")
            return "no battle"
        if mode == "2v2" and not check_for_in_battle_with_delay(controller):
            logger.change_status(status="Not in battle, stopping waiting for 4 elixer.")
            return "no battle"
...
def check_for_4_elixer(controller: BaseEmulatorController) -> bool:
...
def end_fight_state(
    ...
    if get_to_main_after_fight(controller, logger) is False:
    ...
    if not disable_win_tracker_toggle:
        win_check_return = check_if_previous_game_was_win(controller, logger)
...
def check_if_previous_game_was_win(
    ...
    if get_to_activity_log(controller, logger) == "restart":
        logger.change_status(
...
def check_pixels_for_win_in_battle_log(controller: BaseEmulatorController) -> bool:
    ...
    line1 = check_line_for_color(
        controller, x_1=47, y_1=135, x_2=109, y_2=154, color=(255, 51, 102)
    )
    line2 = check_line_for_color(
        controller, x_1=46, y_1=152, x_2=115, y_2=137, color=(255, 51, 102)
    )
    line3 = check_line_for_color(
        controller, x_1=47, y_1=144, x_2=110, y_2=147, color=(255, 51, 102)
    )
...
def find_exit_battle_button(controller: BaseEmulatorController):
...
def find_ok_battle_button2(controller: BaseEmulatorController):
    iar = controller.screenshot()
...
def find_ok_battle_button(controller: BaseEmulatorController):
    if find_ok_battle_button2(controller):
...
def get_to_main_after_fight(controller: BaseEmulatorController, logger):
    timeout = 120  # s
    start_time = time.time()
    ...
    while time.time() - start_time < timeout:
        ...
        # if on clash main
        if check_if_on_clash_main_menu(controller) is True:
        ...
        if not clicked_ok_or_exit:
            ok_button_coord = find_ok_battle_button(controller)
            if ok_button_coord is None:
                ok_button_coord = find_exit_battle_button(controller)
            if ok_button_coord is not None:
                print("Found OK button, clicking it.")
                controller.click(ok_button_coord)
        ...
        if check_for_events_page(controller):
            print("Found events page, clicking clash main button.")
            controller.click((179, 600))
            time.sleep(3)
            continue
        else:
...
def _2v2_fight_loop(
    ...
    # while in battle:
    while check_for_in_battle_with_delay(controller):
        ...
        this_play_side = choose_play_side(logger, controller)
        logger.change_status(
            f"Choose a play side in {str(time.time() - play_choice_start_time)[:4]}s"
        )
        card_indicies = check_which_cards_are_available(controller)
        if card_indicies == []:
        ...
        card_id, play_coord = get_play_coords_for_card(
            controller, card_index, this_play_side
        )
        logger.change_status(
...
def _1v1_fight_loop(
    ...
    while check_if_in_battle(controller):
        ...
        elixer_wait_return = wait_for_4_elixer(controller, logger)
        logger.change_status(
        ...
        this_play_side = choose_play_side_barebones(controller, favorite_side)
        ...
        identification, play_coord = get_play_coords_for_card(
            controller, random_card_index, this_play_side
        )
        logger.change_status(
...
def _2v2_random_fight_loop(controller: BaseEmulatorController, logger: Logger):
    """method to handle a dynamicly timed 2v2 fight"""
    start_time = time.time()
...
def _1v1_random_fight_loop(controller: BaseEmulatorController, logger):
    """method for handling dynamicly timed 1v1 fight"""
