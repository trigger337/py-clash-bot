...
def upgrade_cards_state(controller: BaseEmulatorController, logger: Logger, next_state):
    ...
    controller.click((163, 403))

    # deadspace click to unclick that card but keep the random scroll
    print("Deadspace click")
    controller.click((14, 286))

    # upgrade each card
    for card_index in range(8):
        card_index = 8 - card_index - 1

        while check_if_card_is_upgradable(controller, logger, card_index):
            if not upgrade_card(controller, logger, card_index):
    ...
    controller.click(DEADSPACE_COORD, clicks=3, interval=1.5)

    # return to clash main
    print("Returning to clash main after upgrading")
    controller.click((245, 593))
    time.sleep(3)

    # wait for main
    if wait_for_clash_main_menu(controller, logger, deadspace_click=False) is False:
...
def check_for_second_upgrade_button_condition_1(
    ...
    """
    if not check_line_for_color(controller, 201, 473, 203, 503, (56, 228, 72)):
        return False
    if not check_line_for_color(controller, 275, 477, 276, 501, (56, 228, 72)):
        return False
    if not check_line_for_color(controller, 348, 153, 361, 153, (229, 36, 36)):
        return False

    return True


def check_for_confirm_upgrade_button_condition_1(
    controller: BaseEmulatorController,
) -> bool:
    """
    ...
    Returns:
        bool: True if the condition is met, False otherwise.
    """
    if not check_line_for_color(controller, 201, 401, 201, 432, (56, 228, 72)):
        return False
    if not check_line_for_color(controller, 277, 399, 277, 431, (56, 228, 72)):
        return False
    if not check_line_for_color(controller, 347, 153, 361, 154, (111, 22, 29)):
        return False

    return True


def upgrade_card(controller: BaseEmulatorController, logger: Logger, card_index):
    """
    ...
    if check_for_second_upgrade_button_condition_1(controller):
        controller.click(SECOND_UPGRADE_BUTTON_COORDS_CONDITION_1)
    else:
        controller.click(SECOND_UPGRADE_BUTTON_COORDS)
    time.sleep(2)

    # if gold popup doesnt exists: add to logger's upgrade stat
    if not check_for_missing_gold_popup(controller):
        ...
        if check_for_confirm_upgrade_button_condition_1(controller):
            controller.click(CONFIRM_UPGRADE_BUTTON_COORDS_CONDITION_1)
        else:
            controller.click(CONFIRM_UPGRADE_BUTTON_COORDS)
        time.sleep(2)

        # close card page
        controller.click(CLOSE_CARD_PAGE_COORD)
        time.sleep(2)
    ...
    for _ in range(4):
        controller.click(DEADSPACE_COORD)
        time.sleep(1)
...
def check_for_missing_gold_popup(controller: BaseEmulatorController):
    if not check_line_for_color(
        controller, x_1=338, y_1=215, x_2=361, y_2=221, color=(153, 20, 17)
    ):
        return False
    if not check_line_for_color(
        controller, x_1=124, y_1=201, x_2=135, y_2=212, color=(255, 255, 255)
    ):
        return False

    if not check_line_for_color(controller, 224, 368, 236, 416, (56, 228, 72)):
        return False

    if not region_is_color(controller, [70, 330, 60, 70], (227, 238, 243)):
        return False

    return True


def check_if_card_is_upgradable(
    controller: BaseEmulatorController, logger: Logger, card_index
):
    ...
    if check_if_pixel_indicates_upgradable_card(
        controller.screenshot()[upgrade_coord[1]][upgrade_coord[0]]
    ):
